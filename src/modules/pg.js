...
module.exports = function (pg, palzin, version = null) {
  patchClient(pg.Client, 'pg.Client', palzin)
  ...
  if (getter) {
    ...
    pg.__defineGetter__('native', function () {
      ...
      if (native && native.Client) {
        patchClient(native.Client, 'pg.native.Client', palzin)
      }
      return native
    })
  }
...
function patchClient (Client, klass, palzin) {
  ...
  function wrapQuery (orig, name) {
    return function wrappedFunction (sql) {
      if (palzin.isRecording()) {
        ...
        let segment = palzin.startSegment('pgsql')
